...
def GetCoord2Dict(infile):
    outDict = {}
    with open(infile, 'r') as f:
        for line in f:
            token=line.rstrip('\n').split('\t')
            key = token[0]+':'+token[1]+'-'+token[2]
            print(key)
            outDict[key]=1
    return outDict        
...
#### To get intergenic region indels
## step 1 Get intergenic coords to dict
g = "genome1"
coordsdict = GetCoord2Dict(sys.argv[1])
#PrintDict(coords)
vcfile = "/data2/Manjula/MAGIC16/tmp/vcf/"+g+"_processed/"+g+".nostar.bimultiallele.50bpindel_annotated_col1_9.vcf"
...
gene2svcount=GetIndelCountsForGenes(coordsdict, vcfdict) # genes in a dict
outfile = "/data2/Manjula/MAGIC16/indels_results/"+g+"_intergenic_InDelsStats_Details.txt"
print("Number of intergenic coords from read: ", len(gene2svcount.keys()))
...
with open(sys.argv[2], 'r') as f: ## Read core orthologous file
   ...
   ## send these genes for annotation
    ...
    outfile = "/home/thimmamp/MAGIC16/indels_results/"+g+"_Core_with_"+mode+"_orthogenes_genic_InDelsStats_Details_testing.txt"
