...
mkdir -p $SVcall/Inversion
INV=$SVcall/Inversion
...
### workflow3b
```
cat $SVcall/"$Ref"."$Qry"syri.vcf | awk 'NR==31{print}' > $INV/"$Ref"_"$Qry"syri.vcf.tmp.head
###因为vcf 中的invertion有两个水平，所以我这里sort out了两个level；
###Level1
cat $SVcall/"$Ref"."$Qry"syri.vcf | grep 'PASS' | grep '<INV>' > $INV/"$Ref"_"$Qry"syri.invertion_Level1.tmp1
###Level2
cat $SVcall/"$Ref"."$Qry"syri.vcf | grep 'PASS' | grep '<INVAL>' > $INV/"$Ref"_"$Qry"syri.invertion_Level2.tmp1
###2level together
