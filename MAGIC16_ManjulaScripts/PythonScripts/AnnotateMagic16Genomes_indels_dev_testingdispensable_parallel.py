...
## python AnnotateMagic16Genomes_indels_dev_testingcore.py ~/MAGIC16/annotation/genome1/genes.gff /home/thimmamp/MAGIC16/tmp/vcf/genome1_processed/genome1.nostar.bimultiallele.50bpindel_annotated_col1_9.vcf ~/MAGIC16/gramene_genome33_Only_YongUpdatedFromZhiChao/1.IR64RS2.groupset.sorted.core.16.homologs
            ...
            if not line.startswith('#'):
                ...
                if tokens[2]==field:
                    ...
                    for k,v in gff2shortname.items():
                        if k in key:
                            tmp=key.replace(k,v)
                            if tmp in querygenes.keys(): ## coords for only query genes
                                key1=tmp
                                #print(key, key1)
                    if key1!="":        
                        outDict[key1]=val
...
def GetindelDetailsandcounts(genome, genelist, mode, targetgenome):
    g = genome
    genes_dict=Convert(genelist)
    #print("Number of genes in the query genome ", genome, "  is: ", len(genes_dict.keys()))
    ...
    outfile = "/home/thimmamp/MAGIC16/indels_results/"+targetgenome+"_AbsentGenes_from_"+genome+"_with_mode_"+mode+"_genic_InDelsStats_Details_testing.txt"
    if len(genelist)==len(gene2svcount.keys()):
        print("Number of query genes matches with geneswithsvcount dict\n")
                ...
                for k in row.keys():
                    ...
                    if k!=h and  "Os" in row[k]: #ignore target genome and get its orthologue gene from other genome which is "asbent gene"
                        ...
                        if ',' in row[k]:
                            #g = row[k].split(',')[0]
                            g = row[k].split(',')
                            for item in g: ## multiple orthologous genes
                                genelist.add(item)
                            break;
                        else:
                            genelist.add(row[k])
...
#        ## now get the absent gene's genome and the number of indels from gff and vcf
        ...
        for k1,v1 in genome2absentgenes.items():
            ...
            print(k1,len(v1),'\n')
            GetindelDetailsandcounts(k1, v1, "duplication", h)
...
################################
## To get indels for absent genes
#mode='no_duplication'
#for g in ["Nipponbare"]:
...
with open(sys.argv[4], 'r') as f:
    for line in f:
        ...
        if 'OsNip' in nipid:
            ...
            print(temp[0]+'\t'+temp[1]+'\t'+tokens[1]+'\t'+tokens[2]+'\tNA\tNA\tNA\tNA\tNA')
...
################################
## For no duplication core genes
################################
## usage:
## python AnnotateMagic16Genomes_indels_dev_testingcore_parallel.py "MH63RS3" ~/MAGIC16/gramene_genome33_Only_YongUpdatedFromZhiChao/1.IR64RS2.groupset.sorted.core.16.homologs
##mode=sys.argv[3]
###coreheader = ["MH63RS3", "ZS97RS3", "117425", "125619",	"125827", "127518", "127564", "127652", "127742", "128077", "132278","132424","Azucena","IR64RS2","Nipponbare",	"117534"]
##h = sys.argv[1] ## get the genome header via command line
###print("Number of dispensable genes from each genome with duplication\n")
##genelist=[]
##with open(sys.argv[2], 'r') as f: ## Read core orthologous file
##    reader = csv.DictReader(f, delimiter='\t')
##    for row in reader:
##        if "Os" in row[h]:
##            if ',' in row[h]:
##                #print(row[g], " duplicates ")
##                if mode=="unique":
##                    genelist.append(row[h].split(',')[0])
##                else:
##                    tokens=row[h].split(',')
##                    for t in tokens:
##                        genelist.append(t)
##            else:
##                #print(row[g], " unique ")
##                genelist.append(row[h])
##    g = header2genome[h] 
##    genes_dict = {}
##    for tmp in genelist:
##        genes_dict[tmp]=1
##    print(h+'\t'+header2genome[h]+'\t'+str(len(genelist)))
##   ## send these genes for annotation
##    ## step3 get gene loci and indels within them for each of the dispensable genome's genes
##    annotfile = "/home/thimmamp/MAGIC16/annotation/"+g+"/genes.gff"
##    print("reading annotation file ", annotfile)
...
#genomes =  ["genome1",  "genome5", "genome7", "genome8", "genome9", "genome10", "genome11", "genome12", "genome13", "genome14", "genome15", "genome16", "genome27", "genome28", "genome33"]
''' 
...
querygenesdict = GetCoreGenesForM16Orthologs(sys.argv[3], "all")
print("Number of core genes: ", len(querygenesdict.keys()))
featuredict = GetAnnotationtoDictfromGffForQuery(sys.argv[1], 'gene', querygenesdict)
print("Number of features from gff : ", len(featuredict.keys()))
## get indel counts for each gene in the querygenesdict
gene2svcount=GetIndelCountsForGenes(featuredict, vcfdict) # genes in a dict
outfile = "/home/thimmamp/MAGIC16/indels_results/genome1_Core_genic_InDelsStats_Details_testing.txt"
print("Number of genes from genes2svcount reading: ", len(gene2svcount.keys()))
    #PrintDict(gene2svcount)
WriteDict(gene2svcount, outfile)
'''
...
## Above method is failing to read last column in the file
## hence last column contents copied to a file

temp = GetFile2Dict("genome4_dispensable_genes.txt")
dispgenes2genome={}
for k,v in temp.items():
    if ',' in k:
        ...
        genes = k.split(',')
        for g in genes:
            dispgenes2genome[g]='genome4'
    else:
        dispgenes2genome[k]='genome4'
...
for k,v in dispgenes2genome.items():
    if 'OsN22' in k:
        print(k,v,'\n')
...
for g in ['genome4']:
    ...
    gene2svcount=GetIndelCountsForGenes(featuredict, vcfdict) # genes in a dict
    outfile = "/home/thimmamp/MAGIC16/indels_results/"+g+"_Dispensable_duplication_genic_InDelsStats_Details_testing.txt"
    print("Number of genes from genes2svcount reading: ", len(gene2svcount.keys()))
    #PrintDict(gene2svcount)
    WriteDict(gene2svcount, outfile)
    print("Done writing to file ", outfile, "...")
######### END of Dispensable gene indels
